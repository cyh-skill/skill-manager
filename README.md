...
推送到 `main` 后，GitHub Actions 会在带 Swift 6 的 Apple Silicon `macos-15` runner 上运行测试、构建、独立启动应用并校验 DMG，DMG 可从该次 CI 的 Artifact 下载。推送与 `Resources/Info.plist` 版本一致的标签后，Release 工作流会自动创建 GitHub Release 并上传 DMG 与 SHA-256 文件：
...
通过 GitHub CLI 下载并安装最新 Release：

```bash
release_dir="$(mktemp -d "${TMPDIR:-/tmp}/skill-manager-release.XXXXXX")"
gh release download --repo cyh-skill/skill-manager --pattern '*.dmg' --dir "$release_dir"
open "$release_dir"/Skill-Manager-*.dmg
```
